...
export async function POST(req: NextRequest) {
  try {
    ...
    const tid = 'BTC-' + Date.now().toString(36).toUpperCase() + '-' + Math.random().toString(36).substring(2, 8).toUpperCase();
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to check Bitcoin payment status.' }, { status: 500 });
  }
}