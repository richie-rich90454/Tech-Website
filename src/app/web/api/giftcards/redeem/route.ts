    ...
    try {
        ...
        if (!giftcard) {
            return NextResponse.json(
                { error: 'Invalid or already claimed gift card code.' },
                { status: 400 }
            );
        }
        ...
        if (!plan) {
            return NextResponse.json(
                { error: 'Plan associated with this gift card no longer exists.' },
                { status: 400 }
            );
        }
        ...
        return NextResponse.json({
            success: `Gift card redeemed! You now have the ${plan.name} plan.`,
        });
    } catch (error) {
