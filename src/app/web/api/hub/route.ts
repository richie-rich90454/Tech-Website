...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (!user.membership || user.membership === 0) {
      return NextResponse.json({ error: 'You need an active membership to launch attacks.' }, { status: 403 });
    }
    ...
    if (attackTime > plan.mbt) {
      return NextResponse.json({ error: `Max attack time for your plan is ${plan.mbt}s.` }, { status: 400 });
    }
