...
export default async function AdminDashboardPage() {
  ...
  const stats = [
    ...
    { label: 'Total Revenue', value: `$${revenue.toFixed(2)}`, icon: 'dollar-sign', color: 'info' },
    ...
    <>
      <div className="page-breadcrumb">
        <div className="d-flex align-items-center">
          <h4 className="page-title text-truncate text-white font-weight-medium mb-0">Admin Dashboard</h4>
      ...
      <div className="container-fluid">
        <div className="card-group">
          {stats.map((s, i) => (
            <div key={s.label} className={`card${i < stats.length - 1 ? ' border-right' : ''}`}>
              <div className="card-body">
                <div className="d-flex d-lg-flex d-md-block align-items-center">
                  <div>
                    <div className="d-inline-flex align-items-center">
                      <h2 className="text-white mb-1 font-weight-medium">{s.value}</h2>
                    </div>
                    <h6 className="text-muted font-weight-normal mb-0 w-100 text-truncate mb-2">{s.label}</h6>
                  </div>
                  <div className="ml-auto mt-md-3 mt-lg-0">
                    <span className="opacity-7 text-muted">
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" dangerouslySetInnerHTML={{ __html: icons[s.icon] || '' }} />
                    </span>
                  </div>
                </div>
              </div>
