                        ...
                        <div className="card">
                            <div className="card-body">
                                <div className="d-flex align-items-center mb-3">
                                    ...
                                    <div className="ml-auto">
                                        <a
                                            href="/web/admin/tickets"
                                            className={`btn btn-sm ${filter === 'all' ? 'btn-primary' : 'btn-secondary'} mr-1`}
                                        >
                                            All
                                        </a>
                                        <a
                                            href="/web/admin/tickets?status=open"
                                            className={`btn btn-sm ${filter === 'open' ? 'btn-primary' : 'btn-secondary'} mr-1`}
                                        >
                                            Open
                                        </a>
                                        <a
                                            href="/web/admin/tickets?status=user"
                                            className={`btn btn-sm ${filter === 'user' ? 'btn-primary' : 'btn-secondary'} mr-1`}
                                        >
                                            User Reply
                                        </a>
                                        <a
                                            href="/web/admin/tickets?status=closed"
                                            ...
                                            {tickets.length === 0 ? (
                                                ...
                                                tickets.map((t) => (
                                                    <tr key={t.id}>
                                                        ...
                                                        <td>
                                                            <span
                                                                className={`badge ${t.status === 'Closed' ? 'badge-secondary' : 'badge-warning'}`}
                                                            >
                                                                {t.status}
                                                            </span>
                                                        </td>
                                    ...
                                    <div className="mt-3">
                                        {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                                            (p) => (
                                                <a
                                                    key={p}
                                                    href={`/web/admin/tickets?status=${filter}&page=${p}`}
