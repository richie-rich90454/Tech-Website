...
export function serverError(err: unknown): NextResponse<ApiErr> {
    if (err instanceof AppError) {
        return NextResponse.json<ApiErr>({ ok: false, error: err.message }, { status: err.status });
    }
    return NextResponse.json<ApiErr>({ ok: false, error: toErrorMessage(err) }, { status: 500 });
}
